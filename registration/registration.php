...
function createUserRecord($connectionDatabase, $userName, $userSurname, $userLogin, $userPasswordHash, $avatarSource): int
{
    $createUserQuery = <<<SQL
...
function processRegistration(): void
{
    ...
    $avatarSource = $directoryPath . $fileName;

    if (!move_uploaded_file($avatarFile['tmp_name'], $avatarSource)) {
        http_response_code(500);
        die(json_encode(['error' => 'Не удалось сохранить фото на сервер'], JSON_UNESCAPED_UNICODE));
    }

    $createdUserId = createUserRecord($connectionDatabase, $userName, $userSurname, $userLogin, $userPasswordHash, $avatarSource);
